...
(ns
    ...
    nilern.cljs.algo.generic.comparison
...
;
(defn max
  "Returns the greatest of its arguments. Like cljs.core/max except that
...
(defn min
  "Returns the least of its arguments. Like cljs.core/min except that
   is uses generic comparison functions implementable for any data type."
  ([x] x)
  ([x y] (if (< x y) x y))
  ([x y & more]
